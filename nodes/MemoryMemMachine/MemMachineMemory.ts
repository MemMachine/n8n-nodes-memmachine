...
export class MemMachineMemory {
  ...
  async loadMemoryVariables(_values: InputValues): Promise<MemoryVariables> {
    ...
    if (this.config.cloudTracer && this.config.parentTraceId) {
      loadTraceId = this.config.cloudTracer.startOperation(
        'memory',
        'retrieve',
        {
    ...
    try {
      ...
      if (this.config.cloudTracer && loadTraceId) {
        apiCallTraceId = this.config.cloudTracer.startOperation(
          'memory',
          'search',
          {
      ...
      if (response.ok) {
        this.config.logger?.info(
          '[MemMachineMemory] DEBUG - Search API Success Response:',
          apiResponseBody,
        );
      }
      ...
      if (!response.ok) {
        ...
        const error = new Error(
          `MemMachine API error: ${response.status} ${response.statusText} - ${responseText}`,
        );
      ...
      // Check if response has nested content structure (actual API response)
        ...
        if (content.episodic_memory) {
          ...
          if (
            episodicMem.short_term_memory?.episode_summary &&
            Array.isArray(episodicMem.short_term_memory.episode_summary)
          ) {
            episodeSummary = episodicMem.short_term_memory.episode_summary.filter(
              (s: string) => s && s.trim() !== '',
            );
          }
      ...
      this.config.logger?.info(
        `[MemMachineMemory] DEBUG - Extracted Memories: Total=${rawMemories.length}, Episodic=${rawEpisodicMemory.length}, Semantic=${rawSemanticMemory.length}`,
      );
      ...
      if (this.config.cloudTracer && loadTraceId) {
        processingTraceId = this.config.cloudTracer.startOperation(
          'memory',
          'enrich',
          {
      ...
      if (this.config.enableTemplate && this.config.contextTemplate) {
        const result = this.formatTemplatedMemory(
          rawEpisodicMemory,
          rawProfileMemory,
          rawSemanticMemory,
          episodeSummary,
      ...
      if (Array.isArray(rawEpisodicMemory)) {
        for (const memory of rawEpisodicMemory) {
          ...
          if (
            ...
            const isUserMessage =
              this.config.userId.some((uid: string) => producer && producer.includes(uid)) ||
          ...
          // Handle nested structure (Legacy/Alternative format)
            ...
            if (message.content && message.content.trim() !== '') {
              // Determine if this is a user message or agent message based on producer
              const isUserMessage = this.config.userId.some(
                (uid: string) => message.producer && message.producer.includes(uid),
              );
      ...
      this.config.logger?.info(
        `[MemMachineMemory] DEBUG - Final Messages: Total=${messages.length}, Returned=${recentMessages.length}`,
      );
      ...
      if (loadTraceId && this.config.cloudTracer) {
        ...
        if (this.config.exportToJaeger && this.config.jaegerEndpoint) {
          this.config.cloudTracer
            ...
            .catch((error: Error) => {
              console.error(
                '[MemMachineMemory] Failed to export traces after loadMemoryVariables:',
                error,
              );
            });
        }
        ...
        if (this.config.exportToJaeger && this.config.jaegerEndpoint) {
          this.config.cloudTracer
            ...
            .catch((exportError: Error) => {
              console.error(
                '[MemMachineMemory] Failed to export traces after loadMemoryVariables error:',
                exportError,
              );
            });
        }
  ...
  async saveContext(inputValues: InputValues, outputValues: InputValues): Promise<void> {
    ...
    let saveTraceId = '';
    if (this.config.cloudTracer && this.config.parentTraceId) {
      saveTraceId = this.config.cloudTracer.startOperation(
        'memory',
        'store',
        {
      ...
      if (saveTraceId && this.config.cloudTracer) {
        ...
        if (this.config.exportToJaeger && this.config.jaegerEndpoint) {
          this.config.cloudTracer
            ...
            .catch((exportError: Error) => {
              console.error(
                '[MemMachineMemory] Failed to export traces after saveContext error:',
                exportError,
              );
            });
        }
  ...
  private async storeMessage(
    ...
    if (this.config.cloudTracer && parentTraceId) {
      apiStoreTraceId = this.config.cloudTracer.startOperation(
        'memory',
        'store',
        {
          ...
          'request.body':
            storeRequestBodyFormatted.length > 500
    ...
    if (!response.ok && response.status === 404) {
      ...
      if (errorText.toLowerCase().includes('project')) {
        ...
        if (createProjectResponse.ok || createProjectResponse.status === 409) {
          ...
          throw new Error(
            `Failed to auto-create project: ${createProjectResponse.status} ${createErrorText}`,
          );
        }
      }
    }
    ...
    if (!response.ok) {
      ...
      const error = new Error(
        `MemMachine Store API error: ${response.status} ${response.statusText} - ${responseText}`,
      );
  ...
  private formatTemplatedMemory(
    rawEpisodicMemory: any[],
    rawProfileMemory: any[],
    rawSemanticMemory: any[],
    ...
    this.config.logger?.info(
      '[MemMachineMemory] DEBUG - Processing episodic memories. Raw count:',
      rawEpisodicMemory.length,
    );
    ...
    if (Array.isArray(rawEpisodicMemory)) {
      for (const group of rawEpisodicMemory) {
        ...
        if (Array.isArray(group)) {
          this.config.logger?.info(
            '[MemMachineMemory] DEBUG - Processing memory group size:',
            group.length,
          );
        }
        ...
        for (const item of items) {
          if (item && typeof item === 'object' && item.content && item.content.trim() !== '') {
            ...
            if (!seenEpisodes.has(episodeKey)) {
              ...
              this.config.logger?.info(
                '[MemMachineMemory] DEBUG - Duplicate episode skipped:',
                episodeKey.substring(0, 50) + '...',
              );
            }
          } else {
            this.config.logger?.info(
              '[MemMachineMemory] DEBUG - Invalid or empty episode item encountered:',
    ...
    this.config.logger?.info(
      '[MemMachineMemory] DEBUG - Final flattened memories count:',
    ...
    const shortTermCount =
      this.config.shortTermCount !== undefined ? this.config.shortTermCount : 10;
    ...
    this.config.logger?.info(
      '[MemMachineMemory] Rendered context preview:',
